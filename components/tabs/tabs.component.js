...
function Item({ group, label, value, selected, set }) {
  ...
  return (
    <div
      onClick={update}
      className={`text-white-1 py-2 px-4 font-sans font-bold text-lg rounded-md cursor-pointer ${active} hover:bg-gray-3 transition-colors ease-in mr-2`}>
      {label}
    </div>
  );
}
...
export default function Tabs({ children, group }) {
  ...
  useEffect(() => {
    ...
    for (var child of childs) {
      ...
      if (child.type._name === "Item") items.push({ value: child.props.value, label: child.props.label, jsx: child });
    }
  ...
  // Return markdown
  return (
    <div className={`flex flex-col rounded-lg my-6`}>
      {/* Items container */}
