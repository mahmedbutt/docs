...
To read more about it, click [here](/data/guides/hardware)
...
void updateHandler(const char* path, const char* state) {
    if(strcmp(state, "1") == 0) digitalWrite(2, 1);
...
Here's what'e essentially happening in this code:

![State Diagram](/images/introduction/state-diagram.webp)
...
Add your token, API key and deviceID into the Grandeur Credentials section of the code . Next add enter your WiFi Credentials and you are all good to go! 
...
<Callout emoji="📋">
    <ins>Canvas Guide</ins>](/guides/grandeur-canvas/canvas) 
</Callout>
